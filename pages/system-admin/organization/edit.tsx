...
const EditOrganization = () => {
  ...
  const closeCompletedModal = () => {
    ...
    router.push(
      '/system-admin/organization/detail?organization_id=' + organization_id,
    )
  }
  ...
  const handleUpdate = async () => {
    ...
    if (accounts.length > 0) {
      accounts.forEach((org, index) => {
        // formData.append(`user_orgs[${index}][flag]`, 'UPDATED')
    ...
    try {
      await axiosInstanceSystemAdmin.post(
        `/api/system_admin/1.0/organizations/${organization_id}/update`,
        formData,
      )
